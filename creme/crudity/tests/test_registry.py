...
#         self.assertEqual([f11, f12], ifetcher1.fetchers)
#
...
class CrudityRegistryTestCase(CrudityTestCase):
    def test_empty(self):
        crud_registry = registry.NEWCRUDityRegistry()
        self.assertFalse([*crud_registry.models])

    def test_register(self):
        crud_registry = registry.NEWCRUDityRegistry()
        crud_registry.register(FakeContact, FakeOrganisation)
        self.assertCountEqual(
            [FakeContact, FakeOrganisation],
            [*crud_registry.models],
        )

    # TODO: complete (form descriptors, errors...)
