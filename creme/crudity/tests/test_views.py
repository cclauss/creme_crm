...
from creme.creme_core.models import (  # FakeInvoice FakeImage Job JobResult
    FakeContact,
    FakeOrganisation,
)
...
from ..bricks import (  # CrudityHistoryBrick
    FetcherConfigItemsBrick,
    MachinesConfigItemsBrick,
)
...
class CrudityConfigViewsTestCase(BrickTestCaseMixin, CrudityTestCase):
    def test_portal(self):
        self.login(is_superuser=False, admin_4_apps=['crudity'])
        ...
        create_extr_item = partial(
            MachineConfigItem.objects.create, fetcher_item=fetcher_item,
        )
        create_extr_item(
        ...
        create_extr_item(
            action_type=MachineConfigItem.CRUDAction.UPDATE,
            content_type=FakeContact,
        )
        create_extr_item(
            action_type=MachineConfigItem.CRUDAction.CREATE,
        ...
        response = self.assertGET200(reverse('creme_config__app_portal', args=['crudity']))
        tree = self.get_html_tree(response.content)
        brick_node1 = self.get_brick_node(tree, FetcherConfigItemsBrick.id_)
        self.assertEqual(
            _('%(count)s Fetcher') % {'count': 1},
            self.get_brick_title(brick_node1),
        )

        brick_node2 = self.get_brick_node(tree, MachinesConfigItemsBrick.id_)
        self.assertEqual(
            _('%(count)s Configured resources') % {'count': 2},
            self.get_brick_title(brick_node2),
        )
    ...
    def test_fetcher_item_creation_fs(self):
        self.login(is_superuser=False, admin_4_apps=['crudity'])
        url = reverse('crudity__create_fetcher_item')

        # Step 1 (GET)
        context1 = self.assertGET200(url).context
        get_ctxt = context1.get
        self.assertEqual(_('New fetcher configuration'), get_ctxt('title'))
        self.assertEqual(_('Next step'), get_ctxt('submit_label'))

        with self.assertNoException():
            fetcher_choices = context1['form'].fields['class_id'].choices

        class_id = 'crudity-filesystem'
        self.assertInChoices(
            value=class_id,
            label=_("Files on the server's file system"),
        ...
        # Step 1 (POST)
        step_key = 'fetcher_config_item_creation_wizard-current_step'
        ...
        with self.assertNoException():
            paths_choices = context1_2['form'].fields['path'].choices
        ...
        file_path2 = '/home/creme/crud_import/csv/'
        self.assertInChoices(
            value=file_path2,
            label=file_path2,
            choices=paths_choices,
        ...
        item = self.get_object_or_fail(FetcherConfigItem, class_id=class_id)
        self.assertDictEqual({'path': file_path1}, item.options)
    ...
    def test_fetcher_item_creation_pop(self):
        self.login(is_superuser=False, admin_4_apps=['crudity'])
        url = reverse('crudity__create_fetcher_item')
